...
DotEffect::DotEffect(std::string id, std::string nameKey, std::string typeId)
	: EffectKind(std::move(id), Category::Dot, std::move(nameKey),
				 Stacking::Refresh) {
	m_damageTypeId = std::move(typeId);
}
...
PoisonEffect::PoisonEffect()
	: DotEffect("poison", "effect.poison", "earth") {
...
BleedEffect::BleedEffect()
	: DotEffect("bleed", "effect.bleed", "pierce") {
	m_school = SpellSymbol::Fire;
	m_applyParty = "log.bleeding";
	m_applyMonster = "log.monster_bleeding";
}

BurnEffect::BurnEffect() : DotEffect("burn", "effect.burn", "fire") {
...
DamageType BurnEffect::DamageTypeOf(const Inst& inst) const {
	// Whatever lit it: a fire weapon burns, a water one chills. Which type a
	// school deals is the project's to say, so it is asked rather than known.
	return m_types ? m_types->ForSchool(inst.school) : m_damageType;
}
