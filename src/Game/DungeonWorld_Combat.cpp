	...
	for (const resource::Supply which :
		...
		if (level > 0.0f) {
			...
			if (held) {
				...
				MemberMessage(member,
							  loc::FormatLine(which == resource::Supply::Water
		...
		for (const mishap::Entry& e : table) switch (e.kind) {
			case mishap::Kind::Recover:
				...
				MemberMessage(attacker,
							  loc::FormatLine("log.fumble_recover", attacker.name));
				break;
			case mishap::Kind::Stumble:
				...
				MemberMessage(attacker,
							  loc::FormatLine("log.fumble_stumble", attacker.name));
			...
			case mishap::Kind::SelfHit: {
				...
				MemberMessage(attacker,
							  loc::FormatLine("log.fumble_self", attacker.name));
			...
			case mishap::Kind::Wild: {
				...
				MemberMessage(attacker, loc::FormatLine("log.fumble_wild",
	...
	const auto run = [&](const std::vector<mishap::Entry>& table) {
		for (const mishap::Entry& e : table) switch (e.kind) {
			...
			case mishap::Kind::SelfHit: {
				...
				fx::Deal(ev, self, m_balance.Strike(), m_combatRng);
				if (!monster.Alive())
					onMessage(loc::FormatLine("log.monster_slain",
			...
			case mishap::Kind::Wild: {
				...
				fx::DamageEvent ev = fx::DamageEvent::Blow(atk.type, atk.damage,
														   atk.attackBonus, -1);
				fx::Deal(ev, hit, m_balance.Strike(), m_combatRng);
				onMessage(loc::FormatLine("log.fumble_wild_foe",
										  loc::ViewKey("monster.", monster.kind->name),
									  loc::ViewKey("monster.", beside->kind->name)));
				if (!beside->Alive())
					onMessage(loc::FormatLine("log.monster_slain",
											  loc::ViewKey("monster.", beside->kind->name)));
				break;
...
void DungeonWorld::SpendExertion(Character& member, float points) {
	...
	MemberMessage(member, loc::FormatLine("log.overexert", member.name));
...
void DungeonWorld::MonsterAttack(Monster& monster) {
	...
	if (ev.fumble) {
		MemberMessage(target, loc::FormatLine("log.foe_fumbles", name));
		...
		MemberMessage(target, loc::FormatLine("log.foe_critical", name));
	else if (ev.defenderFumbled)
		MemberMessage(target, loc::FormatLine("log.fumble_guard", target.name));
...
bool DungeonWorld::PartyAttack(size_t member, size_t hand, std::string_view verb) {
	...
	if (ev.fumble) {
		MemberMessage(attacker, loc::FormatLine("log.fumble", attacker.name));
		...
		MemberMessage(attacker, loc::FormatLine("log.critical", attacker.name));
	else if (ev.defenderFumbled)
		MemberMessage(attacker, loc::FormatLine("log.foe_fumbles", name));

	if (!ev.hit) {
		MemberMessage(attacker,
					  loc::FormatLine("log.party_misses", attacker.name, name));
		return finish();
	}
...
void DungeonWorld::BreakableTarget::Absorb(float amount, fx::DamageEvent& ev) {
	...
	if (ev.Quiet()) return; // a tick feeding it is a trickle, not news
	Say(loc::FormatLine("log.monster_absorbs", Name(),
					static_cast<int>(amount + 0.5f)));
}
...
void DungeonWorld::BreakableTarget::SayApplied(const fx::EffectKind& kind) const {
	...
	if (m_world.onMessage)
		m_world.onMessage(loc::FormatLine(kind.ApplyLine(/*onMonster=*/true), Name()));
}
...
void DungeonWorld::NarrateBreak(const BreakableTarget& t,
	...
	if (!onMessage) return;
	if (ev.dealt >= 0.5f)
		onMessage(loc::FormatLine("log.blast_hits", t.Name(),
								  static_cast<int>(ev.dealt + 0.5f)));
		...
		if (m_roster && !m_partyWiped && c.x == m_party.GridX() &&
			...
			for (Character& member : *m_roster) {
				...
				if (ev.dealt >= 0.5f)
					...
					MemberMessage(member, loc::FormatLine("log.member_unharmed",
														  member.name));
				defender.NarrateFall();
				fx::React(ev, defender, nullptr, Reaction());
				if (!active.payload.Empty())
